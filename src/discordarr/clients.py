"""Small, retrying HTTP transport shared by every upstream client.

Ported from shelfmark's ``src/shelfmark_service/clients.py`` (PR #39,
``feat/ebook-download-links``): only ``HttpClient``, ``ServiceError`` and the
circuit breaker they depend on. discordarr does not run Shelfmark's own
worker or API, so ``AudiobookshelfClient``/``ProwlarrClient``/
``QBittorrentClient`` were left behind -- nothing here calls them, and
keeping them would be dead code with its own upstream to track. The
Radarr/Sonarr/Lidarr clients discordarr needs are built on this same
``HttpClient`` (see ``arr_clients.py``), the same way those three were.

The client deliberately returns the upstream JSON shape unchanged.
Normalization belongs in the command layer, because each *arr's response
shape evolves independently of this transport.
"""
...
class CircuitBreakerOpenError(ServiceError):
    ...
    site keeps working unchanged, while code that cares can `isinstance()`
    check for this specifically to tell "we did not even try" apart from "we
    tried and the provider said no".
    """
...
class CircuitBreaker:
    ...
    HttpClient instances are constructed fresh per command invocation, so a
    breaker stored as one of its instance attributes would reset before it
    ever saw a second failure and would never trip. Instances of this class
    are instead looked up by service name from the module-level
    `_BREAKER_REGISTRY` below, so the same breaker is reused across every
    HttpClient built for "radarr" (or any other service) for the life of the
    process.

    A `threading.Lock` guards every state read-and-transition so two threads
    racing on the same breaker cannot both slip through as the single
    half-open trial, or both decide they're the one that trips it open.
    """
...
# Keyed by HttpClient.service (e.g. "radarr"), not by base_url: each service
# only ever has one configured URL, and keying this way is what lets a
# breaker opened by one command's HttpClient still be open for the next
# command's, even though each builds a brand new HttpClient instance.
    ...
    def request(
        ...
        *,
        ...
        request_headers = {
            ...
            "User-Agent": "discordarr/0.1",
        }
        # X-Api-Key is how every *arr app (and Shelfmark's own API) expects
        # its credential -- never sent as a query string, so it cannot end up
        # in a proxy access log or a `/queue` URL echoed back in an error.
        if self.api_key:
        ...
        # breaker how this call turned out -- see the equivalent comment in
        # shelfmark's clients.py for the failure mode this guards against (a
        # half-open probe that raises something with no specific handler
        # would otherwise leave the breaker permanently wedged open).
        resolved = False
        try:
            for attempt in range(self.retries + 1):
                try:
                    ...
                    if not retryable or attempt >= self.retries:
                        ...
                        # one bad API key or a stale item id disable the
                        # provider for every other command behind it.
                        if exc.code >= 500:
